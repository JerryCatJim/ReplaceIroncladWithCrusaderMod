...
using ReplaceIroncladWithCrusader.Code.Audio;
using ReplaceIroncladWithCrusader.Code.Config;
using ReplaceIroncladWithCrusader.Code.Helper;
using ReplaceIroncladWithCrusader.Code.ModConfig;
using System.Collections.Generic;

namespace ReplaceIroncladWithCrusader.Code.Patches;

[HarmonyPatch(typeof(NCreature), "SetAnimationTrigger")]
public static class CrusaderAnimationPatch
{
    public static void Postfix(NCreature __instance, string trigger)
    {
        if (__instance.Entity == null || __instance.Entity.Player == null || __instance.Entity.Player.Character is not Ironclad)
            return;
    ...
    {
        CrusaderHelper.SetBannerAndScriptureVisibility(node, animName);
        CrusaderHelper.SetBladeFlameVisibility(node, animName);

        var visual = node.Visuals;
                ...
                IdleCallable = Callable.From((StringName state) =>
                {
                    if (state == "Idle")
                    {
                        ...
                        if (Idle_SM != null)
                        {
                            ...
                            Idle_SM.Travel(CrusaderHelper.IsLowHealth(node.Entity) && CrusaderSettings.UseLowHealthIdle ? "DeathIdle" : "Idle");
                            CrusaderHelper.SetBannerAndScriptureVisibility(node, "Idle");
                            CrusaderHelper.SetBladeFlameVisibility(node, "Idle");
                        }
            ...
            if (bHasChildStateMachine) //打出卡牌动画先传送到CardPlay状态机再在内部传送到子动画
            {
                        ...
                        if (!string.IsNullOrEmpty(cardAnimName) && cardAnimName != "DoNothing")
                        {
                            ...
                            if (!CrusaderSettings.MuteCardSounds)
                            {
                                CombatAudioManager.PlayCombatSfx("CardSelect/" + cardAnimName,
                                    false, 
                                    false, 
                                    CombatAudioCfg.GetCrusaderVolumeDB("CardSelect/" + cardAnimName)
                                    );
                            }
                        }
                    }
                ...
                else if (animName.Contains("CardPlay/"))
                {
                    ...
                    if (Attack_SM != null)
                    {
                        ...
                        if (CrusaderHelper.ShouldShowBanner(animName) && CrusaderSettings.SwitchBannerImmediately)
                        {
                            state_machine.Start("CardPlay");
                        }
                        else
                        {
                ...
                {
                    if (CrusaderHelper.IsInAnyIdle(animTree, node.Entity))
                    {
                        //已经在某种Idle状态(若角色有多种Idle)时收到取消打出卡牌信号(例如取消打出没有选牌动画的卡牌)后不再调为Idle状态
                        return;
                    }
                    //由信号通知来切换Idle内部状态
    ...
    private static readonly Callable _stateStartedCallable = Callable.From((StringName state) =>
    {
        if (!CrusaderSettings.MuteCardSounds)
        {
            ...
            //把Play音效设置使用第二个单例Audio与选择卡牌的audio区分开，以免连续打出卡牌时上一个Play音效被下一张Select音效打断的问题
            //而且要开启CardPlay状态机结点的AllowTransitionToSelf，否则连续打出相同卡牌时音效和动画不会打断重播
            CombatAudioManager.PlayCombatSfx("CardPlay/" + state, state.ToString().Contains("Recover"), false, VolumeDB, 1); //若为IsTemp，输入的Index不会生效
        }
    });
}

[HarmonyPatch(typeof(HoveredModelTracker), "OnLocalCardSelected")]
public class ReplaceIroncladWithCrusaderOnSelectedPatch
{
    public static void Postfix(CardModel cardModel)
    {
        ...
        if (CrusaderHelper.IsCardOfIronclad(cardModel) && CrusaderHelper.IsIronclad(cardModel.Owner))
        {
            ...
            {
                CreatureCmd.TriggerAnim(cardModel.Owner.Creature, "CardSelect/" + animName, 0);
            }
        }
    }
}
...
public static class ReplaceIroncladWithCrusaderOnCardSelectedPatch
{
    private static CardModel? _lastSelectedCard = null;
    public static void Postfix(NMultiplayerPlayerIntentHandler __instance)
    {
        if (!CrusaderSettings.ShowTeammateCardSelectAnim) return;

        Player? CurrentPlayer = Traverse.Create(__instance).Field("_player").GetValue<Player>();

        if (!CrusaderHelper.IsIronclad(CurrentPlayer) || LocalContext.IsMe(CurrentPlayer)) return;
...
public class ReplaceIroncladWithCrusaderCancelPlayCardPatch
{
...
public class ReplaceIroncladWithCrusaderOnCreatureUnhoverPatch
{
...
public class ReplaceIroncladWithCrusaderAttackCommandPatch
{
...
public class ReplaceIroncladWithCrusaderDeathAnimPatch
{
    public static void Postfix(NCreature __instance)
    {
        if (!CrusaderHelper.IsIronclad(__instance.Entity.Player)) return;
...
public class ReplaceIroncladWithCrusaderReviveAnimPatch
{
    public static void Postfix(NCreature __instance)
    {
        if (!CrusaderHelper.IsIronclad(__instance.Entity.Player)) return;
